...
class LengthRegulator(nn.Module):
    def __init__(self, input_size, duration_kernel_size, device, dropout=0.1, alpha=1.0):
        super().__init__()
    ...
    def forward(self, x, target_duration=None):
        ...
        duration = torch.round(torch.exp(duration) * self.alpha).int()

        lens = duration.sum(dim=-1)
        max_len = int(lens.max())
        mask = torch.zeros((duration.size(0), duration.size(1), max_len))
        for i in range(mask.size(0)):
            tmp = 0
            for j in range(mask.size(1)):
                step = duration[i, j].item()
                mask[i, j, tmp: tmp + step] = 1
                tmp += step
        x = (x.transpose(1, 2) @ mask.to(self.device)).transpose(1, 2)
